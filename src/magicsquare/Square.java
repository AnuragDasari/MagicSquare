...
public class Square {
    ...
    public boolean isMagic() {
        //Add code here!
        //Condition to return false if diagonals are not equal
        if ( sumMainDiag() != sumOtherDiag()){
            return false;
        }
        for( int i = 0; i < square.length; i++){
            //Condition to return false if sum of row or sum of column is not equal to sumMainDiag
            if (sumMainDiag() != sumRow(i) || sumMainDiag() != sumColumn(i)){
                return false;
            }
        }
        return true;
